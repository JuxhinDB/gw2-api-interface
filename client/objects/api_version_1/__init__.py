...
class ItemDetails(BaseAPIObject):
    ...
    def get(self, item_id):
        endpoint_url = self._build_endpoint_url() + '?item_id=%s' % item_id
        response = self.session.get(endpoint_url)
        return response.json()
...
class MapFloor(BaseAPIObject):
    ...
    def get(self, continent_id, floor):
        endpoint_url = self._build_endpoint_url() + '?continent_id=%s&floor=%s' % (continent_id, floor)
        response = self.session.get(endpoint_url)
        return response.json()
...
class RecipeDetails(BaseAPIObject):
    ...
    def get(self, recipe_id):
        endpoint_url = self._build_endpoint_url() + '?recipe_id=%s' % recipe_id
        response = self.session.get(endpoint_url)
        return response.json()
...
class SkinDetails(BaseAPIObject):
    ...
    def get(self, skin_id):
        endpoint_url = self._build_endpoint_url() + '?skin_id=%s' % skin_id
        response = self.session.get(endpoint_url)
        return response.json()
...
class WvWMatchDetails(BaseAPIObject):
    ...
    def get(self, match_id):
        endpoint_url = self._build_endpoint_url() + '?match_id=%s' % match_id
        response = self.session.get(endpoint_url)
        return response.json()
